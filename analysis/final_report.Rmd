...
This code is used to scrap the year and volume from the JASA website:

```{r year_volume_scrap, eval = FALSE}
...
# After having year and volume we scrap the issue.
...
read_articles <- function(volume, issue) {
  ...
  # I needed to scrap the authors from the article specific site
...
# We find the most common word per each year and it's frequency
...
# We first generate a ggplot object and use ggplotly
...
no_authors_year <- jasa_articles %>%
                      select(id, year, author_1:author_19) %>%
                      dplyr::mutate(number_authors = rowSums(!is.na(.)) - 2) %>%
...
finalplot <- no_authors_year %>%
                ...
                labs(title = "Trend of the mean number of authors", 
                     x = "Year", 
...
citation_top10 <- jasa_rm_stopwords %>% 
                    ...
                    dplyr::rename(word = matched_word)
...
citation_top10 %>% 
  ...
  layout(xaxis = list(type = 'category', title = 'Keyword'),
         yaxis = list(title = 'Total Number of Citations'))
