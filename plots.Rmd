...
no_authors_year <- jasa_articles %>%  
  mutate( number_authors = rowSums(!is.na(jasa_articles)) - 1) %>% 
  select(number_authors, year) %>% 
  group_by(year) %>% 
  summarize( Mean_no_authors = mean(number_authors)) 

finalplot <- ggplot(no_authors_year, aes(x = year, y = Mean_no_authors, color = Mean_no_authors)) + geom_point() +
...
```{r}
library(forcats)
jasa_rm_sw = read_csv("./analysis/data/jasa_rm.stopwords.csv")

citation <- jasa_rm_sw %>% 
  select(id, citations, word, title)

citation_top10 <- citation %>% 
  group_by(id) %>% 
  filter( word %in% c("analysis", "bayes", "bootstrap", "clinical", "data", "estimation", "inference", "model", "nonparametric", "regression", "statistics")) %>% 
  select(id, citations, word) %>% 
  group_by(word) %>% 
  summarize( Total_number_of_citations = sum(citations)) 


 citation_top10 %>% 
  mutate( Hot_topics = fct_reorder(word, Total_number_of_citations)) %>% 
