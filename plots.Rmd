...
citation <- jasa_rm_sw %>% 
  dplyr::select(id, citations, word, title)

citation_top10 <- citation %>% 
  group_by(id) %>% 
  dplyr::filter( word %in% c("analysis", "bayes", "bootstrap", "clinical", "data", "estimation", "inference", "model", "nonparametric", "regression", "statistics")) %>% 
  dplyr::select(id, citations, word) %>% 
  group_by(word) %>% 
